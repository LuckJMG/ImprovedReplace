...
class Table(object):
    ...
    def set_table_style(self, table_style, classes):
        ...
        self.colwidths_auto = (
            "colwidths-auto" in classes and "colwidths-given" not in table_style
        ) or ("colwidths-auto" in table_style and ("colwidths-given" not in classes))
    ...
    def get_multicolumn_width(self, start, len_):
        ...
        try:
            mc_width = sum(
                [
                    width
                    for width in ([self._col_width[start + co] for co in range(len_)])
                ]
            )
...
class LaTeXTranslator(nodes.NodeVisitor):
        ...
        if settings.sectnum_xform:  # section numbering by Docutils
            ...
            if secnumdepth is not None:
                ...
                if self.d_class.sections[0] == "part":
                    secnumdepth -= 1
                PreambleCmds.secnumdepth = r"\setcounter{secnumdepth}{%d}" % secnumdepth
    ...
    def visit_table(self, node):
        ...
        self.active_table.set_table_style(self.settings.table_style, node["classes"])
        if "align" in node:
    ...
    def depart_title(self, node):
        self.out.append(self.context.pop())
        if isinstance(node.parent, nodes.table) or node.parent.tagname == "document":
            self.pop_output_collector()
    ...
    def minitoc(self, node, title, depth):
        ...
        self.requirements["minitoc-" + minitoc_name] = r"\do%stoc" % minitoc_name
